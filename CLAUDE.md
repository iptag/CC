...
3. Web Search: Use `searching-with-exa` skill for web search, docs lookup, code examples; use `context7` MCP for library-specific docs.
4. Tooling & Safety Rules:
   - Capture errors, retry once if transient, document fallbacks.
5. Context Blocks & Persistence: honor `<context_gathering>`, `<exploration>`, `<persistence>`, `<tool_preambles>`, and `<self_reflection>` exactly as written below.
6. Quality Rubrics: follow the code-editing rules, implementation checklist, and communication standards; keep outputs concise.
7. Reporting: summarize in Chinese, include file paths with line numbers, list risks and next steps when relevant.
